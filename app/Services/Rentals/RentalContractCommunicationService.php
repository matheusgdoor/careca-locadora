...
final class RentalContractCommunicationService
{
    ...
    public function sendEmail(RentalContract $contract): void
    {
        ...
        $signatureUrl = $signatureRequest ? $this->signatures->url($signatureRequest) : null;
        ...
        Mail::html($html, function (Message $message) use ($email, $contract, $pdf, $filename): void {
            $message
                ->to($email)
                ->subject("Contrato de locação {$contract->number}")
    ...
    public function whatsappUrl(RentalContract $contract): string
    {
        $contract->loadMissing('customer');
        $phone = preg_replace('/\D+/', '', (string) ($contract->customer?->whatsapp ?: $contract->customer?->phone));

        if (blank($phone)) {
            throw new RuntimeException('O cliente não possui WhatsApp/telefone cadastrado.');
        }
        ...
        $text = "Olá, {$contract->customer?->display_name}. Segue o contrato de locação {$contract->number} da Careca Locadora. "
            . ($signatureRequest
    ...
    private function latestPendingRequest(RentalContract $contract): ?RentalContractSignatureRequest
    {
